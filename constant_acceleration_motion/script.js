...
function drawScene(t) {
  ...
  if (sel === "all") {
    savedPaths.forEach((p, idx) => {
      ...
      // If a kinematic equation is selected, hide all vectors and show only equation vectors
      // Do not force-show equation vectors when previewing 'all' saved paths
      if (getKinematicEquation() && sel !== "all") {
        if (getKinematicEquation() === "eq1") {
          // Show only equation 1 vectors
          try {
            const p0 = p.data && p.data.length ? p.data[0] : null;
            const vx0 = p0 ? p0.vx || 0 : 0;
            const vy0 = p0 ? p0.vy || 0 : 0;
            const tsec = s.t;
            const gmag = Math.abs(s.g || (p0 && p0.g) || 9.81);
            const at_m = gmag * tsec; // magnitude of a*t (m/s)
            const vs = vectorScale;
            const at_pixels_y = at_m * vs; // positive => down on canvas
            drawArrow(
              ctx,
              bx,
              by,
          ...
          // Show only equation 2 vectors (independent of the Position toggle)
          const p0 = p.data && p.data.length ? p.data[0] : null;
          ...
          drawArrow(ctx, originX, originY, dxr, dyr, "#2e7d32", "s", true);
        }
        // If user enables other vectors in Vector Analysis, show them as well
        if (document.getElementById("velVec").checked) {
          ...
          if (mode === "all" || mode === "resultant") {
            if (horizMode) drawArrow(ctx, bx, by, s.vx * vs, 0, "blue", "v");
            else drawArrow(ctx, bx, by, s.vx * vs, -s.vy * vs, "blue", "v");
          }
          if (!oneD_local) {
            if (
              mode === "all" ||
              ...
              drawArrow(ctx, bx, by, s.vx * vs, 0, "rgba(0,0,255,0.4)", "vx");
            if (mode === "all" || mode === "vertical" || mode === "components")
              drawArrow(ctx, bx, by, 0, -s.vy * vs, "rgba(0,0,255,0.4)", "vy");
          }
        }
        if (document.getElementById("accelVec").checked) {
          const aScale = (vectorScale / 2.5) * 3; // scale acceleration visuals with vectorScale
          const ax = s.ax !== undefined ? s.ax : 0;
          const ay = s.ay !== undefined ? s.ay : -s.g;
          if (horizMode) drawArrow(ctx, bx, by, ax * aScale, 0, "#d32f2f", "a");
          else
            drawArrow(ctx, bx, by, ax * aScale, -ay * aScale, "#d32f2f", "a");
        }
        ...
        if (
          ...
          // show position vector when user requested it (independent of selected equation)
          // ...existing code for position vector...
        }
      } else {
        // ...existing code...
        if (document.getElementById("showTangent").checked && s.v_res > 0) {
        ...
        if (document.getElementById("velVec").checked) {
          const mode = document.getElementById("vectorMode").value,
            vs = vectorScale;
          if (mode === "all" || mode === "resultant")
            drawArrow(ctx, bx, by, s.vx * vs, -s.vy * vs, "blue", "v");
          if (!hideEqComponents) {
            if (
              mode === "all" ||
              mode === "horizontal" ||
    ...
    // (baseH0 already includes visible saved paths heights)
  } else if (sel && sel.startsWith("p")) {
